...
## 2020-3-11 
> 本次更新内容:增加内置DNS，并写入netflix规则，指定路由
由于技术有限，有部分错误
* 内置DNS端口变为随机，需手动修改为53
* 端口 port 前面缩进不规范，需手动对其，一般为空格
  第一个 port 前面为8个空格 
  第二个 port 前面为6个空格
注意:修改完成保存后重启v2ray服务端即可
## 2019-12-30 
...
```
wget -N --no-check-certificate -q -O install.sh "https://raw.githubusercontent.com/steamsv/V2Ray_ws-tls_bash_onekey/master/install.sh" && chmod +x install.sh && bash install.sh
```
### 启动方式
